#define DT_DRV_COMPAT zmk_behavior_alt_repeat
...
static uint32_t keycode_substitution(uint32_t keycode) {
    switch (keycode) {
        case RIGHT_SHIFT: return H;
...
static int on_key_repeat_binding_pressed(struct zmk_behavior_binding *binding,
    ...
    data->current_keycode_pressed.keycode =
        keycode_substitution(data->current_keycode_pressed.keycode);
